...
def get_metadata_from_dicom(img_dicom):
    metadata = { #windowing 할 때 필요한 것
        "window_center": img_dicom.WindowCenter, #window center
        "window_width": img_dicom.WindowWidth, #window width
        "intercept": img_dicom.RescaleIntercept, #rescale intercept
        "slope": img_dicom.RescaleSlope, #rescale slope
    }b
...
# three window 
# def apply_window_policy(img, row):
#     image1 = misc.apply_window(image, 40, 80, intercept, slope)
#     image2 = misc.apply_window(image, 80, 200, intercept, slope)
#     image3 = misc.apply_window(image, 600, 2800, intercept, slope)
