...
class RSNA_Dataset_train_window3_by_study_context(data.Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        image_cat = cv2.resize(image, (512, 512)) #bone
        ...
        #     image_cat = self.transform(image_cat)
            ...
            #image_cat = torch.Tensor(augmented['image'][np.newaxis])
...
def random_shift(image, max_translate=(0.2, 0.2)):
    ...
    image = transforms.RandomAffine(image, translate=(w_t, h_t), shear=0, scale=1)
    return image
...
def aug_image(image, is_infer=False):
    if is_infer:
        ...
        image = randomHorizontalFlip(image, u = 0.5) #default
        image = randomVerticleFlip(image, u = 0.5)
        #image = random_shift(image, max_translate=(0.5, 0.5)) #random shift
