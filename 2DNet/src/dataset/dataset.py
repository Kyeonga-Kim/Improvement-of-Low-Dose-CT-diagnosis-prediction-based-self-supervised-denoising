...
#         image = cv2.resize(image, (512, 512)) #array
...
#         study_train_df = self.df[self.df['study_instance_uid']==study_name] # study_intance_uid가 같은 얘들 df
...
#         image = cv2.imread(train_png_dir  + filename, 0) # 0 : read grayscale
...
#         # image_cat.save('/home/kka0602/RSNA2019_Intracranial-Hemorrhage-Detection/2DNet/default_window/' + filename.replace('.png','') + '.png')
...
class RSNA_Dataset_train_window3_by_study_context(data.Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        filename = self.name_list[idx % len(self.name_list)]  #fold별 저장해논 train filename batchsize(16)씩 
        filename_train_df = self.df[self.df['filename']==filename]
        study_name = filename_train_df['study_instance_uid'].values[0]
        study_index = int(filename_train_df['slice_id'].values[0].split('_')[-1])
        study_train_df = self.df[self.df['study_instance_uid']==study_name] # study_intance_uid가 같은 얘들 df
        ...
        # image_cat = cv2.imread('/home/dataset/RSNA_3window/3_window/' + filename)
        # image_cat = cv2.resize(image_cat, (512, 512))

        image = cv2.imread('/home/dataset/RSNA_3window/3_window/' + filename, cv2.IMREAD_COLOR)
        image = cv2.resize(image[:,:,0], (512, 512)) #brain
    
        image_up = cv2.imread('/home/dataset/RSNA_3window/3_window/' + filename_up,  cv2.IMREAD_COLOR)
        image_up = cv2.resize(image_up[:,:,0], (512, 512))

        image_down = cv2.imread('/home/dataset/RSNA_3window/3_window/' + filename_down,  cv2.IMREAD_COLOR)
        image_down = cv2.resize(image_down[:,:,0], (512, 512))

        image_cat = np.concatenate([image_up[:,:,np.newaxis], image[:,:,np.newaxis], image_down[:,:,np.newaxis]], 2)

        # image_cat = PIL.Image.fromarray(image_cat)  
        
        #image_cat.save('/home/kka0602/RSNA2019_Intracranial-Hemorrhage-Detection/brain_window/' + filename_down)
        #image_cat = np.concatenate([image_down, image, image_up], -1) #(512,512,9)
        

        label = torch.FloatTensor(self.df[self.df['filename']==filename_down].loc[:, 'any':'subdural'].values)
        image_cat = aug_image(image_cat, is_infer=True)
    
        if self.transform is not None:
            augmented = self.transform(image=image_cat)
            image_cat = augmented['image'].transpose(2, 0, 1) #(9,256,256)
...
class RSNA_Dataset_val_window3_by_study_context(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image_cat = PIL.Image.fromarray((image_cat * 255).astype(np.uint8))  
        #image_cat.save('/home/kka0602/nas/data/brain_3window/' + filename.replace('.dcm','.png'))
        ...
        if self.transform is not None:
            ...
            augmented = self.transform(image=image_cat)     
            image_cat = augmented['image'].transpose(2, 0, 1) #(256,256,3) -> (3,256,256)
...
def aug_image(image, is_infer=False):
    if is_infer:
        image = randomHorizontalFlip(image, u=0) #좌우반전
...
def generate_dataset_loader2(df_all, c_train, train_transform, train_batch_size, c_val, val_transform, val_batch_size, workers):
